...
tape('node/expression/get-attribute', (test) => {
    test.test('constructor', (test) => {
        ...
        let node = new GetAttributeExpressionNode(expr, attr, args, Template.ARRAY_CALL, 1, 1);

        test.same(node.getNode('node'), expr);
        test.same(node.getNode('attribute'), attr);
        test.same(node.getNode('arguments'), args);
        test.same(node.getAttribute('type'), Template.ARRAY_CALL);
    ...
    test.test('compile', (test) => {
        ...
        let node = new GetAttributeExpressionNode(expr, attr, args, Template.ANY_CALL, 1, 1);
        ...
        node = new GetAttributeExpressionNode(expr, attr, args, Template.ARRAY_CALL, 1, 1);
        ...
        node = new GetAttributeExpressionNode(expr, attr, args, Template.METHOD_CALL, 1, 1);
